...
def preprocess(img: np.ndarray, model_img_size: int) -> np.ndarray:
    ...
    interpolation = cv2.INTER_LINEAR if ratio > 1.0 else cv2.INTER_AREA  # LINEAR is ~3x faster than LANCZOS4
...
def extract_bbox_coordinates(detection: Dict) -> Optional[Tuple[float, float, float, float]]:
    ...
    # Handle both list [x, y, w, h] and dict {"x":, "y":, "width":, "height":} formats

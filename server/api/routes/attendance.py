...
from api.dependencies import get_attendance_db, get_face_db
...
# Public router (accessible by students/kiosk for listing)
router = APIRouter(prefix="/attendance", tags=["attendance"])

# Admin router (protected by API key for management)
admin_router = APIRouter(prefix="/attendance", tags=["attendance"], dependencies=[Depends(verify_api_key)])
...
@admin_router.post("/groups", response_model=AttendanceGroupResponse)
...
@admin_router.put("/groups/{group_id}", response_model=AttendanceGroupResponse)
    ...
    try:
        ...
        for field, value in updates.dict(exclude_unset=True).items():
            if field == "settings" and value:
                ...
                # Allow clearing room_id by sending empty string or null
...
@admin_router.delete("/groups/{group_id}", response_model=SuccessResponse)
...
@admin_router.post("/members", response_model=AttendanceMemberResponse)
...
@admin_router.post("/members/bulk", response_model=BulkMemberResponse)
        ...
        for member_data in bulk_data.members:
            try:
                ...
                if not group:
                    errors.append(
                        {
                            "person_id": member_data.person_id or member_data.name,
                ...
                # Auto-generate person_id if not provided (Fix for bulk import)
                if not db_member_data.get("person_id"):
                    db_member_data["person_id"] = generate_person_id(
                        name=member_data.name, db=db, group_id=member_data.group_id
                    )
                ...
                if success:
                    ...
                    errors.append(
                        {
                            "person_id": db_member_data["person_id"],
                ...
                errors.append({"person_id": member_data.person_id or member_data.name, "error": str(e)})
                error_count += 1
...
@admin_router.get("/members", response_model=List[AttendanceMemberResponse])
async def get_all_members(
    db: AttendanceDatabaseManager = Depends(get_attendance_db),
):
    """Get all attendance members across all groups"""
    try:
...
async def search_members(
    ...
    """Search all members across all groups by name or enrollment number"""
    logger.info(f"[Search] Searching for '{q}' (limit={limit})")
    results = db.search_all_members(q, limit)
    logger.info(f"[Search] Found {len(results)} results for '{q}'")
    return {"results": results, "count": len(results)}


@admin_router.post("/members/assign")
async def assign_member(
    person_id: str = Query(..., description="Person ID to assign"),
    group_id: str = Query(..., description="Target group ID"),
    db: AttendanceDatabaseManager = Depends(get_attendance_db),
):
    """Assign an existing member to a different group"""
    # Verify group exists
    group = db.get_group(group_id)
    if not group:
    ...
    if not db.assign_member_to_group(person_id, group_id):
        raise HTTPException(status_code=500, detail="Failed to assign member")

    return {
        "success": True,
        "message": f"Member {member.get('name', person_id)} assigned to {group.get('name', group_id)}",
    }
...
@admin_router.put("/members/{person_id}", response_model=AttendanceMemberResponse)
...
@admin_router.delete("/members/{person_id}", response_model=SuccessResponse)
...
async def cleanup_database(
    ...
    success = db.cleanup_old_data(days_to_keep=days)
    if not success:
        raise HTTPException(status_code=500, detail="Failed to cleanup database")
    return {"success": True}


@admin_router.post("/maintenance/vacuum")
...
@admin_router.post("/maintenance/factory-reset")
...
async def create_subject(
    group_id: str = Query(..., description="Group ID to add subject to"),
    name: str = Query(..., description="Subject name"),
    db: AttendanceDatabaseManager = Depends(get_attendance_db),
):
    """Create a subject for a group"""
    group = db.get_group(group_id)
    if not group:
        raise HTTPException(status_code=404, detail="Group not found")

    subject_id = generate_id()
    result = db.add_subject({"id": subject_id, "group_id": group_id, "name": name})
    if not result:
        raise HTTPException(status_code=500, detail="Failed to create subject")
    return {"success": True, "subject": result}


@admin_router.get("/subjects/{group_id}")
async def get_subjects(
    group_id: str,
    db: AttendanceDatabaseManager = Depends(get_attendance_db),
):
    """Get all subjects for a group"""
    return db.get_subjects(group_id)
...
@admin_router.post("/subjects/{subject_id}/members")
async def assign_subject_members(
    subject_id: str,
    data: SubjectMemberAssignment,
    db: AttendanceDatabaseManager = Depends(get_attendance_db),
):
    """Assign students to a specific subject"""
    success_count = 0
    for person_id in data.person_ids:
        if db.assign_student_to_subject(person_id, subject_id):
            success_count += 1
    
    return {"success": True, "count": success_count}


@admin_router.get("/subjects/{subject_id}/members")
async def get_subject_members(
    subject_id: str,
    db: AttendanceDatabaseManager = Depends(get_attendance_db),
):
    """Get all students enrolled in a subject"""
    members = db.get_subject_members(subject_id)
    return {"results": members, "count": len(members)}


@admin_router.delete("/subjects/{subject_id}/members/{person_id}")
async def remove_subject_member(
    ...
    if not db.remove_student_from_subject(person_id, subject_id):
        raise HTTPException(status_code=404, detail="Membership not found")
    return {"success": True, "message": "Member removed from subject"}

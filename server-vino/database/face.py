...
class FaceDatabaseManager:
    ...
    def __init__(self, database_path: str):
        ...
        # Configuration for multi-embedding averaging
        self.max_embeddings_per_person = 5  # Store up to 5 embeddings, average them
    ...
    def _initialize_database(self):
        ...
        try:
            with self._get_connection() as conn:
                ...
                # Create faces table (stores averaged/primary embedding)
                cursor.execute(
                    """
                ...
                # Create face_embeddings table for multi-embedding storage
                cursor.execute(
                    """
                    CREATE TABLE IF NOT EXISTS face_embeddings (
                        ...
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                        FOREIGN KEY (person_id) REFERENCES faces(person_id) ON DELETE CASCADE
                    )
                """
                )
                
                # Add angle column if it doesn't exist (migration)
                try:
                    cursor.execute(
                        """
                        ALTER TABLE face_embeddings ADD COLUMN angle TEXT DEFAULT 'front'
                        """
                    )
                ...
                # Create index for face_embeddings
                cursor.execute(
                    """
    ...
    def _compute_averaged_embedding(self, embeddings: List[np.ndarray]) -> np.ndarray:
        """
        Compute averaged embedding from multiple embeddings.
        The average is then L2-normalized for cosine similarity matching.
        
        Args:
            embeddings: List of embedding arrays
            
        Returns:
            Normalized averaged embedding
        """
        if len(embeddings) == 1:
            return embeddings[0]
    ...
    def add_person(self, person_id: str, embedding: np.ndarray) -> bool:
        ...
        Only stores the latest embedding (replaces any existing one).
        ...
        try:
            with self.lock:
                with self._get_connection() as conn:
                    ...
                    # Remove any existing embeddings for this person in face_embeddings table
                    cursor.execute(
                        """
                    ...
                    # Add the latest embedding to face_embeddings table
                    cursor.execute(
                        """
                        INSERT INTO face_embeddings (person_id, embedding, embedding_dimension)
                        VALUES (?, ?, ?)
                        """,
                        (person_id, embedding_blob, embedding_dim),
                    )
                    
                    # Update or insert the embedding in faces table (single latest embedding)
                    cursor.execute(
                        """
                    ...
                    conn.commit()
                    logger.info(f"Stored latest embedding for {person_id}")
                    return True
    ...
    def get_persons_by_ids(self, person_ids: List[str]) -> Dict[str, np.ndarray]:
        """
        Batch-load face embeddings for multiple person IDs in a single query.
        
        Args:
            person_ids: List of person IDs to look up
            
        Returns:
            Dict mapping person_id → embedding (only includes IDs that exist)
        """
        if not person_ids:
            return {}
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(
                    f"SELECT person_id, embedding FROM faces WHERE person_id IN ({placeholders})",
                    person_ids,
                )
                return {
                    row["person_id"]: self._blob_to_embedding(row["embedding"])
                    for row in cursor.fetchall()
                }
        except Exception as e:
            logger.error(f"Failed to batch-get persons: {e}")
            return {}

    def remove_person(self, person_id: str) -> bool:
        """
        Remove a person from the database (both faces and face_embeddings tables)
        ...
        try:
            with self.lock:
                with self._get_connection() as conn:
                    ...
                    # Remove from face_embeddings first (due to foreign key)
                    cursor.execute(
                        """
                        DELETE FROM face_embeddings WHERE person_id = ?
                        """,
    ...
    def get_embedding_count(self, person_id: str) -> int:
        ...
        Returns:
            Number of embeddings stored for this person
        """
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
            ...
            logger.error(f"Failed to get embedding count for {person_id}: {e}")
            return 0

    def has_face(self, person_id: str) -> bool:
        """
        Check if a person has a registered face embedding.
        
        Args:
            person_id: Unique identifier for the person
            
        Returns:
            bool: True if person has face data, False otherwise
        """
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
            ...
            logger.error(f"Failed to check face for {person_id}: {e}")
            return False
    ...
    def add_person_with_angle(
        self, person_id: str, embedding: np.ndarray, angle: str = "front"
    ) -> bool:
        """
        Add a person's face embedding with angle metadata.
        ...
        try:
            with self.lock:
                with self._get_connection() as conn:
                    ...
                    # First ensure the person exists in faces table (for FK constraint)
                    cursor.execute(
                        """
                        INSERT OR IGNORE INTO faces (person_id, embedding, embedding_dimension)
                        VALUES (?, ?, ?)
                        """,
                        (person_id, embedding_blob, embedding_dim),
                    )
                    
                    # Add to face_embeddings table with angle
                    cursor.execute(
                        """
                        INSERT INTO face_embeddings (person_id, embedding, embedding_dimension, angle)
                        VALUES (?, ?, ?, ?)
                        """,
                        (person_id, embedding_blob, embedding_dim, angle),
                    )
                    ...
                    # Compute averaged embedding from all angles
                    embeddings = [self._blob_to_embedding(row["embedding"]) for row in rows]
                    averaged_embedding = self._compute_averaged_embedding(embeddings)
                    averaged_blob = self._embedding_to_blob(averaged_embedding)
                    
                    # Update the averaged embedding in faces table
                    cursor.execute(
                        """
                        UPDATE faces SET embedding = ?, embedding_dimension = ?, updated_at = CURRENT_TIMESTAMP
                        WHERE person_id = ?
                        """,
                        (averaged_blob, embedding_dim, person_id),
                    )

                    conn.commit()
                    logger.info(f"Added {angle} embedding for {person_id}, total embeddings: {len(embeddings)}")
                    return True

        except Exception as e:
            logger.error(f"Failed to add person {person_id} with angle {angle}: {e}")
            return False

    def get_all_person_embeddings(self, person_id: str) -> List[Dict]:
        """
        Get all embeddings for a person (all angles).

        Args:
            person_id: Unique identifier for the person

        Returns:
            List of dicts with embedding and angle info
        """
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                ...
                cursor.execute(
                    """
                    SELECT embedding, angle, created_at FROM face_embeddings 
                    WHERE person_id = ?
                    ORDER BY created_at DESC
                    """,
                    (person_id,),
            ...
            logger.error(f"Failed to get embeddings for {person_id}: {e}")
            return []

    def get_all_persons_with_all_embeddings(self) -> Dict[str, List[np.ndarray]]:
        """
        Get all persons with ALL their embeddings (not averaged).
        Used for best-of-N matching in multi-angle recognition.
            ...
            with self._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if person_id not in persons:
                        persons[person_id] = []
                    persons[person_id].append(embedding)

                return persons

        except Exception as e:
            logger.error(f"Failed to get all persons with embeddings: {e}")
            return {}

    def clear_person_embeddings(self, person_id: str) -> bool:
        """
        Clear all embeddings for a person (used before re-registration).
        ...
        try:
            with self.lock:
                with self._get_connection() as conn:
                    ...
                    conn.commit()
                    logger.info(f"Cleared all embeddings for {person_id}")
                    return True

        except Exception as e:
            logger.error(f"Failed to clear embeddings for {person_id}: {e}")
            return False

...
class LivenessDetector:
    def __init__(
        ...
        # For antisymmetric model outputs [real, -real], use threshold 0
        # (real_logit > 0 means real face)
    ...
    def detect_faces(
        ...
        # Crop faces in BGR first, then convert each crop to RGB
        # (avoids converting the entire image when only small crops are needed)
        face_crops, valid_detections, skipped_results = (
            extract_face_crops_from_detections(
                image,  # Pass BGR image directly
                valid_detections_for_cropping,
                self.bbox_inc,
                self.increased_crop,
            )
        ...
        # Convert crops to RGB (much cheaper than full-image conversion)
        face_crops = [cv2.cvtColor(c, cv2.COLOR_BGR2RGB) for c in face_crops]
